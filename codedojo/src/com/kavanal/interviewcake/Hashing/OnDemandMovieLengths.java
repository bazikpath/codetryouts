...
public class OnDemandMovieLengths{

    public static boolean canTwoMoviesFillFlight(int[] movieLengths, int flightLength) {

        // determine if two movies add up to the flight length
        Set<Integer> seenMovieLengths = new HashSet<>();

        for (int firstMovieLength : movieLengths) {
            int remainingMovieLength = flightLength - firstMovieLength;
            if (seenMovieLengths.contains(remainingMovieLength))
                return true;

            seenMovieLengths.add(firstMovieLength);
        }
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(OnDemandMovieLengths.class);
